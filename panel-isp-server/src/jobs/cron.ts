...
async function suspendExpiredUnpaid(): Promise<void> {
    ...
    for (const row of rows) {
        ...
        try {
            ...
            await recordAudit({
                actor: 'system',
                action: 'pelanggan.auto_suspend',
                targetType: 'pelanggan',
                targetId: pelangganId,
                targetName: String(pel.nama),
                summary: `Auto-suspend "${String(pel.nama)}" karena masa aktif habis & belum lunas`,
                meta: { tanggalExpire: row.tanggalExpire },
            });
        } catch (error: unknown) {
...
export function startCronJobs(): void {
    ...
    logger.info(`Cron jobs aktif (timezone ${CRON_TZ})`);
}
