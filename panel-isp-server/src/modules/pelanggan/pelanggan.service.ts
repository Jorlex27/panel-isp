...
export interface ListPelangganParams {
    q?: string;
...
export interface ListPelangganResult {
    rows: PelangganPopulated[];
    total: number;
    page: number;
    limit: number;
    totalPages: number;
...
export async function listPelanggan(params: ListPelangganParams): Promise<ListPelangganResult> {
    const match: Record<string, unknown> = {};
    if (params.status) match.status = params.status;
    if (params.q) {
        const rx = { $regex: escapeRegex(params.q), $options: 'i' };
        match.$or = [{ nama: rx }, { ipAddress: rx }, { noHp: rx }, { macAddress: rx }];
    }
    ...
    const agg = await col()
        .aggregate<{ rows: PelangganPopulated[]; total: { n: number }[] }>([
            { $match: match },
            { $sort: { nama: 1 } },
            {
                $facet: {
                    rows: [{ $skip: skip }, { $limit: limit }, ...pelangganPopulatePipeline],
                    total: [{ $count: 'n' }],
                },
            },
        ])
...
export async function createPelanggan(
    ...
    try {
        ...
        await recordAudit({
            actor,
            action: 'pelanggan.create',
            targetType: 'pelanggan',
            targetId: created._id,
            targetName: created.nama,
            summary: `Menambah pelanggan "${created.nama}" (IP ${created.ipAddress}, paket ${created.paket?.nama ?? '—'})`,
            after: { ip: created.ipAddress, mac: created.macAddress, paket: created.paket?.nama, status: created.status, statusBayar },
        });
        return created;
    } catch (error: unknown) {
...
export async function suspendPelangganDb(id: ObjectId, actor: string): Promise<PelangganPopulated> {
    ...
    await recordAudit({
        actor,
        action: 'pelanggan.suspend',
        targetType: 'pelanggan',
        targetId: id,
        targetName: refreshed.nama,
        summary: `Men-suspend pelanggan "${refreshed.nama}" (IP ${refreshed.ipAddress})`,
        before: { status: prev },
        after: { status: 'suspend' },
    });
    return refreshed;
}

export async function aktifkanPelangganDb(id: ObjectId, actor: string): Promise<PelangganPopulated> {
    const pel = await getPelanggan(id);
    const prev = pel.status;
    ...
    await recordAudit({
        actor,
        action: 'pelanggan.aktifkan',
        targetType: 'pelanggan',
        targetId: id,
        targetName: refreshed.nama,
        summary: `Mengaktifkan pelanggan "${refreshed.nama}" (IP ${refreshed.ipAddress})`,
        before: { status: prev },
        after: { status: 'aktif' },
    });
    return refreshed;
...
export async function deletePelangganDb(id: ObjectId, actor: string): Promise<void> {
    const pel = await getPelanggan(id);
    ...
    await recordAudit({
        ...
        targetId: id,
        targetName: pel.nama,
        summary: `Menghapus pelanggan "${pel.nama}" (IP ${pel.ipAddress}, MAC ${pel.macAddress})`,
        before: { ip: pel.ipAddress, mac: pel.macAddress, paket: pel.paket?.nama, status: pel.status },
    });
}
...
export async function bayarPelanggan(
    ...
    const pel = await getPelanggan(id);
    const lang = await langgananService.recordBayar(id, body.jumlah, body.metode, actor);
    let diaktifkan = false;
    ...
    await recordAudit({
        actor,
        action: 'pelanggan.bayar',
        targetType: 'pelanggan',
        targetId: id,
        targetName: pel.nama,
        summary: `Mencatat pembayaran Rp ${body.jumlah.toLocaleString('id-ID')} (${body.metode}) untuk "${pel.nama}"${diaktifkan ? ', otomatis diaktifkan' : ''}`,
        meta: {
            jumlah: body.jumlah,
            metode: body.metode,
            berlakuSampai: lang.tanggalExpire,
            diaktifkan,
        },
    });
...
export async function gantiPaket(
    ...
    await recordAudit({
        actor,
        action: 'pelanggan.ganti_paket',
        targetType: 'pelanggan',
        targetId: id,
        targetName: out.nama,
        summary: `Mengganti paket "${out.nama}": ${paketLama} → ${out.paket?.nama ?? '—'}`,
        before: { paket: paketLama },
        after: { paket: out.paket?.nama },
    });
    return out;
}
...
export async function gantiMacPelanggan(
    ...
    await recordAudit({
        actor,
        action: 'pelanggan.ganti_mac',
        targetType: 'pelanggan',
        targetId: id,
        targetName: out.nama,
        summary: `Mengganti MAC "${out.nama}": ${pel.macAddress} → ${normalized}`,
        before: { mac: pel.macAddress },
        after: { mac: normalized },
    });
    return out;
}
