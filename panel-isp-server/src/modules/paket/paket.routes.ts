...
export const paketRouter = new Hono()
    ...
    .post('/', async c => {
        ...
        const data = await paketService.createPaket(body, c.get('actor'));
        return c.json({ success: true, data }, 201);
    })
    .patch('/:id', async c => {
        const id = parseObjectId(c.req.param('id'), 'ID paket');
        const body = paketUpdateSchema.parse(await c.req.json());
        const data = await paketService.updatePaket(id, body, c.get('actor'));
        return c.json({ success: true, data });
    })
    .put('/:id', async c => {
        const id = parseObjectId(c.req.param('id'), 'ID paket');
    ...
    .delete('/:id', async c => {
        const id = parseObjectId(c.req.param('id'), 'ID paket');
        await paketService.deletePaket(id, c.get('actor'));
